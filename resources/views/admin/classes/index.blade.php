...
<style>
  ...
  .tablex{ width:100%; border-collapse:separate; border-spacing:0; min-width:720px; }
  ...
  @media (max-width: 640px){
    ...
    .tablex tbody tr > td{ display:grid; grid-template-columns: 110px 1fr; gap:8px; border:0; padding:8px 0; text-align:left; }
...
<div class="page">
  <div class="wrap">
    ...
    <div class="card">
      <div class="card-body">
        <div class="scroll-x">
          <table id="classesTable" class="tablex align-middle">
            <thead>
              <tr>
                <th style="width:6rem">#</th>
                <th>Class</th>
                <th style="width:14rem">Monthly Fee</th>
                <th class="ta-right" style="width:16rem">Actions</th>
              </tr>
            </thead>
            <tbody>
              @php $start = (is_object($classes) && method_exists($classes,'firstItem')) ? $classes->firstItem() : 1; @endphp
              @forelse($classes as $c)
                <tr data-row="class">
                  <td class="num text-muted" data-label="#">{{ $start + $loop->index }}</td>
                  <td class="fw-semibold" data-col="name" data-label="Class">{{ $c->name }}</td>
                  <td class="num" data-label="Monthly Fee">
                    @if(!is_null($c->fee))
                      PKR {{ number_format($c->fee, 0) }}
                    @else
              ...
              @if($classes->count() > 0)
                <tr id="noSearchRow" style="display:none;">
                  <td colspan="4" class="empty">No matches for your search.</td>
                </tr>
              @endif
            </tbody>
          </table>
    ...
    @if(is_object($classes) && method_exists($classes,'hasPages') && $classes->hasPages())
...
<script>
  // Realtime client-side filter by class name (works on the current page) + renumber visible rows
  (function(){
    ...
    const rows = Array.from(table.tBodies[0].querySelectorAll('tr[data-row="class"]'));
    ...
    function filter(){
      ...
      rows.forEach(tr => {
        ...
        const match = norm(name).includes(term);
