...
@section('content')
@php
    // Ensure $student is always defined so nullsafe works on create page
...
<div class="container" style="max-width:900px">
    ...
    <form action="{{ $isEdit ? route('students.update', $student?->id) : route('students.store') }}"
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control"
                           value="{{ $isEdit ? ($student?->reg_no ?? '') : 'Auto on save' }}"
                           disabled>
                </div>
                ...
                <div class="col-md-6">
                    <label class="form-label">{{ $isEdit ? 'Change Password (users table, optional)' : 'Password (users table)' }}</label>
            ...
            {{-- Add in Class --}}
            @php
                $addClass = old('add_class', $student?->class_id !== null);
            @endphp
            <div class="mb-3">
            ...
            {{-- Add in Course --}}
            @php
                $addCourse = old('add_course', $student?->course_id !== null);
            @endphp
            <div class="mb-3">
                <div class="form-check mb-2">
                ...
                <select name="course_id" id="course_id" class="form-control" style="display:none;">
                    ...
                    @foreach($courses as $co)
                        <option value="{{ $co->id }}"
                            {{ old('course_id', $student?->course_id) == $co->id ? 'selected' : '' }}>
                            {{ $co->name }}
                        </option>
                    @endforeach
                </select>
