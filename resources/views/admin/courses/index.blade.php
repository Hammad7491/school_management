...
<div class="page">
  <div class="wrap">
    ...
    <div class="card">
      <div class="card-body">
        <div class="scroll-x">
          <table id="coursesTable" class="tablex align-middle">
            <thead>
              <tr>
                  ...
                  <td data-label="Image">
                    @if($course->image_path)
                      <div class="img-actions">
                        <img src="{{ asset('storage/'.$course->image_path) }}" alt="course image" class="thumb">
                        <a href="{{ asset('storage/'.$course->image_path) }}" download class="btn-sm">Download</a>
                        <a href="{{ asset('storage/'.$course->image_path) }}" target="_blank" class="btn-sm">Open</a>
                      </div>
                    @else
    ...
    @if(is_object($courses) && method_exists($courses,'hasPages') && $courses->hasPages())
      <div style="margin-top:14px;">{{ $courses->links() }}</div>
    @endif
...
<script>
  // Realtime client-side filter by course name (works on the current page) + renumber visible rows
  (function(){
    ...
    const rows = Array.from(tbody.querySelectorAll('tr[data-row="course"]'));
