  ...
  <div class="wrap">
    ...
    <div class="bar">
      <div>
        ...
        <div id="resultMeta" style="color:var(--muted); font-size:12px;"><span id="resultCount">{{ $homeworks->count() }}</span> on this page</div>
      </div>
      <form class="search" role="search" onsubmit="return false">
        ...
        <input id="q" type="search" placeholder="Search by class, course, comment, creator…" autocomplete="off" />
            ...
            <tbody>
              ...
              @forelse($homeworks as $h)
                ...
                <tr data-row="hw">
                  ...
                  <td class="fw-semibold" data-col="day" data-label="Day">{{ \Carbon\Carbon::parse($h->day)->format('Y-m-d') }}</td>
                  <td data-col="class" data-label="Class">{{ $h->schoolClass->name ?? '—' }}</td>
                  <td data-col="course" data-label="Course">{{ $h->course->name ?? '—' }}</td>
                  <td data-col="comment" data-label="Comment" class="text-truncate" style="max-width: 420px;">{{ $h->comment ?? '—' }}</td>
                  <td data-label="File">
                    @if($fileUrl)
                      <div class="file-actions">
                        <span class="chip">{{ pathinfo($fileName, PATHINFO_EXTENSION) ? strtoupper(pathinfo($fileName, PATHINFO_EXTENSION)) : 'FILE' }}</span>
              ...
              @empty
                <tr>
                  <td colspan="8" class="empty">No homeworks found. <a href="{{ route('homeworks.create') }}" style="text-decoration:underline">Add your first homework</a>.</td>
                </tr>
              @endforelse
