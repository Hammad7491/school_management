...
@section('content')
@php
  $total = (is_object($exams) && method_exists($exams,'total'))
            ? $exams->total()
            : (is_countable($exams) ? count($exams) : 0);
@endphp
...
<div class="page">
  <div class="wrap">
    ...
    <div class="bar">
      <div>
        <h1 class="title">Exam <span>Schedule</span></h1>
      ...
      <div class="search">
        ...
        <input id="exSearch" type="search" placeholder="Search by class, course or comment…">
      </div>
    </div>
    ...
    <div class="cardx">
          ...
          <tbody id="exBody">
            @forelse($exams as $e)
              ...
              <tr class="ex-row">
                ...
                <td data-label="Comment">{{ $cmt }}</td>
                <td data-label="File">
                  @if($e->file_path)
                    <a class="btn-link" href="{{ route('exams.download',$e->id) }}">
            ...
            @empty
              <tr>
                <td colspan="4" class="empty">No exams yet.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
      ...
      @if(is_object($exams) && method_exists($exams,'hasPages') && $exams->hasPages())
        <div class="footer">{{ $exams->links() }}</div>
      @endif
    </div>
...
<script>
  (function(){
    ...
    const rows = Array.from(document.querySelectorAll('#exBody .ex-row'));

    function filter() {
      const term = q.value.trim().toLowerCase();
      rows.forEach(row => {
        const hay = row.textContent.toLowerCase();   // class + course + comment + file text
